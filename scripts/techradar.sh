...
grep -q '"mongoose"' "$PKG" 2>/dev/null && found "Mongoose" "MongoDB ODM"
grep -q '"knex"' "$PKG" 2>/dev/null && found "Knex" "Query builder"
grep -q '"drizzle-orm"' "$PKG" 2>/dev/null && found "Drizzle" "ORM"
grep -q '"kysely"' "$PKG" 2>/dev/null && found "Kysely" "Type-safe SQL"
...
grep -q '"@reduxjs/toolkit"\|"redux"\|"react-redux"' "$PKG" 2>/dev/null && found "Redux/RTK" ""
grep -q '"zustand"' "$PKG" 2>/dev/null && found "Zustand" ""
grep -q '"@ngrx"' "$PKG" 2>/dev/null && found "NgRx" "Angular"
echo "$FILES" | xargs grep -l "BehaviorSubject\|ReplaySubject" 2>/dev/null | head -1 | grep -q . && found "RxJS Subjects" "Reactive state"
...
grep -q '"pinia"' "$PKG" 2>/dev/null && found "Pinia" "Vue"
grep -q '"mobx"' "$PKG" 2>/dev/null && found "MobX" ""
grep -q '"jotai"' "$PKG" 2>/dev/null && found "Jotai" "Atomic state"
grep -q '"recoil"' "$PKG" 2>/dev/null && found "Recoil" ""
echo ""
...
echo "$FILES" | xargs grep -l "oauth\|OAuth\|oauth2\|openid\|oidc\|OpenID" 2>/dev/null | head -1 | grep -q . && found "OAuth2/OIDC" ""
echo "$FILES" | xargs grep -l "keycloak\|Keycloak\|KEYCLOAK" 2>/dev/null | head -1 | grep -q . && found "Keycloak" "SSO/IAM"
...
# Cross-repo auth hints (proxy configs, env vars)
grep -rl "auth.*proxy\|/auth/\|/token\|/oauth" "$REPO"/*.js "$REPO"/*.json "$REPO"/proxy* 2>/dev/null | head -1 | grep -q . && found "Auth proxy" "Backend handles auth (check proxy config)"
echo ""
