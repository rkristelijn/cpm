...
check 1 "Has test script/config" "grep -q 'test' '$REPO/package.json' 2>/dev/null || [ -f '$REPO/jest.config.js' ] || [ -f '$REPO/vitest.config.ts' ] || grep -q '^test:' '$REPO/Makefile' 2>/dev/null || find '$REPO' -name '*Test.java' -maxdepth 5 2>/dev/null | head -1 | grep -q . || find '$REPO' -name '*_test.go' -maxdepth 5 2>/dev/null | head -1 | grep -q . || ([ -f '$REPO/pom.xml' ] && find '$REPO' -path '*/src/test/*' -maxdepth 5 2>/dev/null | head -1 | grep -q .)"
...
check 2 "Has test files" "find '$REPO' -name '*.test.*' -o -name '*.spec.*' -o -name '*_test.*' -o -name '*Test.java' -o -name '*IT.java' 2>/dev/null | grep -v node_modules | head -1 | grep -q ."
check 2 "Has CHANGELOG" "[ -f '$REPO/CHANGELOG.md' ]"
check 2 "Has conventional commits" "[ -f '$REPO/commitlint.config.js' ] || [ -f '$REPO/.commitlintrc.json' ] || grep -q 'commitizen\|commitlint\|standard-version\|semantic-release' '$REPO/package.json' 2>/dev/null || (cd '$REPO' && git log --oneline -20 2>/dev/null | grep -cE '^[a-f0-9]+ (feat|fix|chore|docs|refactor|test|ci|perf)' | grep -qv '^0$')"
