...
struct EnvConfigCheck : Check {
  EnvConfigCheck() { name = "env-config"; category = "security"; }

  std::vector<Finding> run(FileSystem& fs, ToolRunner&) override {
    std::vector<Finding> findings;

    /* Dangerous env patterns */
    static const struct { const char* pattern; const char* msg; } dangerous[] = {
    ...
    const char* config_files[] = {"docker-compose.yml", "docker-compose.yaml",
        ".env", ".env.production", "Dockerfile", nullptr};
    ...
    for (int i = 0; config_files[i]; i++) {
      ...
      for (int j = 0; dangerous[j].pattern; j++) {
        ...
        if (content.find(dangerous[j].pattern) != std::string::npos)
          findings.push_back({name, "error", config_files[i], 0, "dangerous-env",
              dangerous[j].msg, "Remove or restrict to development only", ""});
      }
    }
