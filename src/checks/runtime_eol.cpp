...
struct RuntimeEolCheck : Check {
  ...
  std::vector<Finding> run(FileSystem& fs, ToolRunner&) override {
    ...
    if (node_ver > 0 && node_ver < node_min)
      findings.push_back({name, "error", ".nvmrc", 0, "node-eol", "Node.js " + std::to_string(node_ver) + " is EOL",
    ...
    if (fs.exists(".python-version")) {
      ...
      if (std::regex_search(content, m, re)) {
        ...
        if (ver < py_min)
          findings.push_back(
              {name, "warning", ".python-version", 0, "python-eol", "Python " + m[0].str() + " approaching EOL", "Upgrade to 3.11+"});
      }
    }
    ...
    if (fs.exists("pom.xml")) {
      ...
      if (std::regex_search(content, m, re)) {
        ...
        if (ver < 17)
          findings.push_back({name, "error", "pom.xml", 0, "java-eol", "Java " + m[1].str() + " is EOL", "Upgrade to Java 17+"});
      }
    }
