...
struct ArchitectureCheck : Check {
  ...
  ArchitectureCheck() { name = "architecture"; category = "quality"; }
...
private:
  void check_nesting(const std::string& file, const std::string& content, std::vector<Finding>& findings) {
    ...
    for (size_t i = 0; i < content.size(); i++) {
      if (content[i] == '\n') { line++; continue; }
      if (content[i] == '{') { depth++; if (depth > max_depth) { max_depth = depth; worst_line = line; } }
      if (content[i] == '}') { if (depth > 0) depth--; }
    }
  ...
  void check_fan_out(const std::string& file, const std::string& content, std::vector<Finding>& findings) {
    int imports = 0;
    size_t pos = 0;
    while ((pos = content.find("import ", pos)) != std::string::npos) { imports++; pos += 7; }
    pos = 0;
    while ((pos = content.find("#include", pos)) != std::string::npos) { imports++; pos += 8; }
  ...
  void check_switch_chains(const std::string& file, const std::string& content, std::vector<Finding>& findings) {
    ...
    while (pos < content.size()) {
      ...
      if (ln.find("switch") != std::string::npos || ln.find("match ") != std::string::npos) {
        in_switch = true; cases = 0; switch_line = line;
      }
      if (in_switch && (ln.find("case ") != std::string::npos || ln.find("else if") != std::string::npos))
        cases++;
  ...
  void check_infra_coupling(const std::string& file, const std::string& content, std::vector<Finding>& findings) {
    ...
    if (file.find("domain") == std::string::npos &&
        file.find("service") == std::string::npos &&
        file.find("core") == std::string::npos) return;

    static const char* infra[] = {"mysql", "postgres", "redis", "mongodb", "prisma",
        "typeorm", "sequelize", "knex", "mongoose", "firebase", nullptr};
