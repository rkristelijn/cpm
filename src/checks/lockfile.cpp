...
struct LockfileCheck : Check {
  LockfileCheck() { name = "lockfile"; category = "deps"; }

  std::vector<Finding> run(FileSystem& fs, ToolRunner&) override {
    std::vector<Finding> findings;

    struct { const char* manifest; const char* locks[4]; const char* mgr; } checks[] = {
      {"package.json", {"package-lock.json", "yarn.lock", "pnpm-lock.yaml", nullptr}, "npm/yarn/pnpm"},
    ...
    for (int i = 0; checks[i].manifest; i++) {
      ...
      for (int j = 0; checks[i].locks[j]; j++) {
        if (fs.exists(checks[i].locks[j])) { found = true; break; }
      }
      if (!found) {
