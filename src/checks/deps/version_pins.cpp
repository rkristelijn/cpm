  ...
  std::vector<Finding> run(FileSystem& fs, ToolRunner&) override {
    ...
    if (fs.exists("package.json")) {
      ...
      for (size_t i = 0; i < content.size(); i++) {
        if ((content[i] == '^' || content[i] == '~') && i > 0 && content[i - 1] == '"') count++;
      }
      if (count > 0)
        findings.push_back({name, "warning", "package.json", 0, "unpinned-npm", std::to_string(count) + " unpinned deps (^ or ~)",
    ...
    if (fs.exists("Dockerfile")) {
      ...
      if (content.find(":latest") != std::string::npos)
        findings.push_back({name, "warning", "Dockerfile", 0, "latest-tag", "Using :latest tag", "Pin to specific version"});
    }
    ...
    if (fs.exists(".github/workflows/ci.yml")) {
      ...
      if (content.find("@main") != std::string::npos || content.find("@master") != std::string::npos)
        findings.push_back(
            {name, "warning", ".github/workflows/ci.yml", 0, "unpinned-action", "GitHub Action pinned to branch (use SHA)", ""});
    }
