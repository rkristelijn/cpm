...
struct CryptoCheck : Check {
  CryptoCheck() { name = "crypto"; category = "security"; }

  std::vector<Finding> run(FileSystem& fs, ToolRunner&) override {
    std::vector<Finding> findings;

    static const struct { const char* pattern; const char* rule; const char* msg; const char* fix; } patterns[] = {
      {"SSLv2", "weak-ssl", "SSLv2 is broken", "Use TLS 1.2+"},
    ...
    for (auto& file : files) {
      ...
      for (int i = 0; patterns[i].pattern; i++) {
        ...
        if (pos != std::string::npos) {
          int line = 1;
          for (size_t j = 0; j < pos; j++) if (content[j] == '\n') line++;
          findings.push_back({name, "error", file, line, patterns[i].rule,
              patterns[i].msg, patterns[i].fix, "https://cpm.dev/checks/crypto"});
        }
      }
    }
    return findings;
