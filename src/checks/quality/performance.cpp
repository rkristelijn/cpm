  ...
  std::vector<Finding> run(FileSystem& fs, ToolRunner&) override {
    ...
    for (auto& file : files) {
      ...
      while (pos < content.size()) {
        ...
        /* Track loop context */
        if (ln.find("for ") != std::string::npos || ln.find("for(") != std::string::npos || ln.find(".map(") != std::string::npos ||
            ln.find(".forEach(") != std::string::npos || ln.find("while ") != std::string::npos)
          in_loop = true;
        if (in_loop && ln.find("}") != std::string::npos && ln.find("{") == std::string::npos) in_loop = false;

        /* N+1: async/fetch/db inside loop */
        if (in_loop) {
          if (ln.find("await ") != std::string::npos || ln.find("fetch(") != std::string::npos)
            findings.push_back({name, "warning", file, line, "n-plus-one", "Async/fetch inside loop (potential N+1)",
                                "Batch queries or use Promise.all", ""});
          if (ln.find(".find(") != std::string::npos || ln.find(".filter(") != std::string::npos)
            findings.push_back({name, "info", file, line, "nested-lookup", "Collection lookup inside loop (potential O(n²))",
        ...
        if (ln.find("await ") != std::string::npos) {
          ...
          if (sequential_awaits >= 3)
            findings.push_back({name, "info", file, line, "sequential-await", "3+ sequential awaits — consider Promise.all()",
        ...
        if (ln.find("readFileSync") != std::string::npos || ln.find("writeFileSync") != std::string::npos)
          findings.push_back(
              {name, "warning", file, line, "sync-io", "Synchronous file I/O (blocks event loop)", "Use async fs.readFile()", ""});

        /* JSON clone anti-pattern */
        if (ln.find("JSON.parse(JSON.stringify(") != std::string::npos)
          findings.push_back({name, "info", file, line, "json-clone", "JSON.parse(JSON.stringify()) is slow for cloning",
                              "Use structuredClone() or spread", ""});

        /* Catastrophic regex: nested quantifiers */
        if ((ln.find("(.*)*") != std::string::npos || ln.find("(a+)+") != std::string::npos || ln.find("(.+)+") != std::string::npos))
          findings.push_back({name, "error", file, line, "regex-catastrophic", "Regex with nested quantifiers (catastrophic backtracking)",
                              "Rewrite regex or add timeout", ""});
        ...
        if ((ln.find("cache[") != std::string::npos || ln.find("cache.set(") != std::string::npos) &&
          ...
          findings.push_back({name, "info", file, line, "unbounded-cache", "Cache without size limit or TTL",
                              "Add maxSize/TTL to prevent memory growth", ""});
