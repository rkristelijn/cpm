    ...
    for (auto& file : files) {
      ...
      while (pos < content.size()) {
        ...
        if ((file.find("page") != std::string::npos || file.find("layout") != std::string::npos ||
             file.find("index.html") != std::string::npos) &&
            line == 1) {
          if (content.find("<title") == std::string::npos && content.find("metadata") == std::string::npos &&
            ...
            findings.push_back({name, "info", file, 0, "seo-no-meta", "Page without title/meta tags — bad for SEO",
        ...
        if (ln.find("new Date(") != std::string::npos &&
          ...
          findings.push_back({name, "info", file, line, "reinvent-dates", "Manual date manipulation — use date-fns or dayjs",
        ...
        if (ln.find("86400000") != std::string::npos || ln.find("* 24 * 60 * 60") != std::string::npos)
          findings.push_back({name, "info", file, line, "reinvent-date-math", "Manual date arithmetic (ms calculation)",
        ...
        if ((ln.find("language === ") != std::string::npos || ln.find("locale === ") != std::string::npos) &&
          ...
          findings.push_back({name, "info", file, line, "reinvent-i18n", "Manual language switching — use i18n library",
                              "Use next-intl, react-i18next, or @angular/localize", ""});

        /* === Logging smells === */
        /* console.log in production code */
        if (ln.find("console.log(") != std::string::npos && file.find("debug") == std::string::npos &&
            file.find("logger") == std::string::npos)
          findings.push_back({name, "info", file, line, "console-log-prod", "console.log in production code",
        ...
        if (ln.find("console.") != std::string::npos && ln.find("JSON.stringify") != std::string::npos)
          findings.push_back({name, "warning", file, line, "log-json-stringify", "JSON.stringify in log statement — expensive at scale",
                              "Use structured logger that handles serialization", ""});

        /* === Dead links in JSX === */
        if (ln.find("href=\"#\"") != std::string::npos || ln.find("href='#'") != std::string::npos)
          findings.push_back(
              {name, "info", file, line, "dead-link", "href='#' — dead link or placeholder", "Use proper route or button for actions", ""});
        ...
        if (ln.find("from 'moment'") != std::string::npos || ln.find("require('moment')") != std::string::npos)
          findings.push_back({name, "warning", file, line, "bundle-moment", "moment.js is 300KB+ and deprecated",
