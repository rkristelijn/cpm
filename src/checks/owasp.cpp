...
struct OwaspCheck : Check {
  OwaspCheck() { name = "owasp"; category = "security"; }

  std::vector<Finding> run(FileSystem& fs, ToolRunner&) override {
    std::vector<Finding> findings;

    static const struct { const char* pattern; const char* rule; const char* msg; const char* fix; const char* owasp; } rules[] = {
    ...
    for (auto& file : files) {
      ...
      for (int i = 0; rules[i].pattern; i++) {
        ...
        while ((pos = content.find(rules[i].pattern, pos)) != std::string::npos) {
          ...
          findings.push_back({name, "warning", file, line, rules[i].rule,
              std::string("[") + rules[i].owasp + "] " + rules[i].msg,
