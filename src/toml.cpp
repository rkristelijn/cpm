...
static void defaults(CpmConfig *cfg) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        if (strcmp(section, "project") == 0) {
            ...
            if (strcmp(key, "pre-commit") == 0) cfg->hook_pre_commit = bval;
            else if (strcmp(key, "pre-push") == 0) cfg->hook_pre_push = bval;
            else if (strcmp(key, "commit-msg") == 0) cfg->hook_commit_msg = bval;
...
CpmTool *cpm_tool_find(CpmConfig *cfg, const char *name) {
    for (int i = 0; i < cfg->tool_count; i++)
        if (strcmp(cfg->tools[i].name, name) == 0)
            return &cfg->tools[i];
    return NULL;
}
...
CpmCheck *cpm_check_find(CpmConfig *cfg, const char *name) {
    for (int i = 0; i < cfg->check_count; i++)
        if (strcmp(cfg->checks[i].name, name) == 0)
            return &cfg->checks[i];
    return NULL;
}

/* Default config paths (config_dir/filename) */
struct CfgDefault { const char *key; const char *file; };
static const CfgDefault CFG_DEFAULTS[] = {
...
const char *cpm_config_path(CpmConfig *cfg, const char *key) {
    ...
    for (int i = 0; i < cfg->config_count; i++)
        if (strcmp(cfg->configs[i].key, key) == 0)
            return cfg->configs[i].path;
    ...
    for (int i = 0; CFG_DEFAULTS[i].key; i++) {
        if (strcmp(CFG_DEFAULTS[i].key, key) == 0) {
            ...
            snprintf(bufs[i], sizeof(bufs[i]), "%s/%s",
                     cfg->config_dir, CFG_DEFAULTS[i].file);
            return bufs[i];
        }
    }
    return NULL;
