...
int scan_universal(Repo& repo) {
  ...
  if (is_software && !has_file(repo.path, "Makefile") && !has_file(repo.path, "makefile") &&
  ...
  if (!has_file(repo.path, "README.md") && !has_file(repo.path, "readme.md")) {
    ...
    if (rf) {
      ...
      if (strstr(rbuf, "Getting started with GitLab") ||
        ...
        repo.findings_warnings++;
        total++;
        finding_write(name, "readme-audit", "warning", "README.md",
                      "default-readme", "README is still the default template");
      }
      ...
      if (strcasestr(rbuf, "install") || strcasestr(rbuf, "setup") ||
          ...
          strcasestr(rbuf, "how to run") || strcasestr(rbuf, "usage"))
        score++;
      ...
      if (strcasestr(rbuf, "prerequisite") || strcasestr(rbuf, "requirement") ||
          ...
          strcasestr(rbuf, "tech stack"))
        score++;
      if (strcasestr(rbuf, "contribut") || strcasestr(rbuf, "development") ||
          strcasestr(rbuf, "pull request") || strcasestr(rbuf, "PR"))
        score++;
  ...
  if (is_software) {
    bool has_tests = has_file(repo.path, "tests") || has_file(repo.path, "test") ||
                     has_file(repo.path, "__tests__") || has_file(repo.path, "spec");
    ...
    if (!has_tests) {
      repo.findings_warnings++;
      total++;
      finding_write(name, "testing", "warning", ".", "no-tests",
      ...
      if (fgets(b, sizeof(b), p)) {
        ...
        if (months > 12) {
          ...
          snprintf(msg, sizeof(msg),
                   "Last commit %ld months ago — consider archiving", months);
  ...
  if (!has_file(repo.path, ".gitignore")) {
    repo.findings_warnings++;
    total++;
    finding_write(name, "devops", "warning", ".", "no-gitignore",
                  "No .gitignore — likely committing build artifacts");
  }
  ...
  if (is_software) {
    // Bus factor
    std::string cmd_authors =
        "git -C " + shell_escape(repo.path) +
        " shortlog -sn --since='6 months ago' 2>/dev/null | wc -l";
      ...
      if (fgets(b, sizeof(b), pc)) {
        ...
        if (changes > 20) {
          ...
          if (*fname) {
            ...
            repo.findings_warnings++;
            total++;
            finding_write(name, "git-health", "warning", fname,
                          "high-churn", msg);
          }
        }
      }
      ...
      if (pl) {
        ...
        if (fgets(b, sizeof(b), pl)) {
          ...
          if (files_changed > 50) {
            ...
            repo.findings_warnings++;
            total++;
            finding_write(name, "git-health", "warning", ".", "large-commit",
                          msg);
          }
        }
        pclose(pl);
  ...
  if (!has_file(repo.path, "SECURITY.md") &&
      !has_file(repo.path, ".github/SECURITY.md")) {
    repo.findings_warnings++;
    total++;
  ...
  if (is_software) {
    ...
    if (stat(issue_dir.c_str(), &st) != 0 &&
        stat(gitlab_issue.c_str(), &st) != 0) {
      repo.findings_warnings++;
      total++;
    ...
    if (stat(pr_tmpl.c_str(), &st) != 0 &&
        stat(pr_tmpl2.c_str(), &st) != 0 &&
        stat(gl_mr.c_str(), &st) != 0) {
      repo.findings_warnings++;
      total++;
  ...
  {
    std::string cmd = "git -C " + shell_escape(repo.path) +
                      " ls-files .env 2>/dev/null | head -1";
    FILE* p = popen(cmd.c_str(), "r");
    if (p) {
      char b[256];
      if (fgets(b, sizeof(b), p) && b[0] && b[0] != '\n') {
        repo.findings_warnings++;
        total++;
        finding_write(name, "security", "warning", ".env", "env-committed",
                      ".env file tracked in git — likely contains secrets");
      }
      pclose(p);
    }
  }
