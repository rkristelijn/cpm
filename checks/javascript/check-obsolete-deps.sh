...
declare -A HEAVY=(
  [lodash]="Use @flupkejs/lodash (drop-in, native internals) or native Array/Object methods"
  [moment]="Use @flupkejs/moment (drop-in, uses Temporal) or native Intl.DateTimeFormat"
  ...
  [node-uuid]="Use @flupkejs/uuid (drop-in) or crypto.randomUUID() — native since Node 15.6"
  [uuid]="Use @flupkejs/uuid (drop-in) or crypto.randomUUID() — native since Node 15.6"
  [node-fetch]="Use native fetch() — built into Node 18+"
  [cross-fetch]="Use native fetch() — built into Node 18+"
  [axios]="Use @flupkejs/axios (drop-in, uses native fetch) or fetch() directly"
